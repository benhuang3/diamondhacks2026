...
DEMO_FINDINGS_TEMPLATE: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "severity": "medium",
        "category": "a11y",
        "title": "Featured shoe image missing alt text",
        "description": "The hero running-shoe image has no alt attribute, so screen readers announce only the filename (IMG_2398.jpg).",
        "suggestion": 'Add descriptive alt text, e.g. alt="Men\'s Nike Air Zoom Pegasus 40 in navy blue, three-quarter view".',
    },
    {
        ...
        "severity": "medium",
        "category": "a11y",
        "title": "Product search input lacks visible label",
        "description": "The search field (placeholder 'Search shoes, brands, sizes…') has no associated <label>, making it inaccessible to screen readers.",
        "suggestion": "Add a visually hidden <label for='search'>Search shoes and brands</label>.",
    },
    {
        "selector": "a.nav-link",
        ...
        "severity": "low",
        "category": "nav",
        "title": "Category nav links have invisible focus state",
        "description": "Keyboard focus ring on Men/Women/Kids nav links is suppressed via `outline: none` without a replacement indicator.",
        "suggestion": "Add `:focus-visible { outline: 2px solid #3366FF; outline-offset: 2px; }`.",
    },
    {
        "selector": "select.size-picker",
        "xpath": "/html/body/main/div[3]/form/select",
        "bounding_box": {"x": 820, "y": 340, "w": 140, "h": 36},
        "severity": "high",
        "category": "a11y",
        "title": "Size picker lacks associated label",
        "description": "The size <select> dropdown ('US 9', 'US 9.5', etc.) has no visible or aria-label, so assistive tech announces it as 'combobox' with no context.",
        "suggestion": "Add `<label for='size'>Select size (US)</label>` above the dropdown.",
    },
    {
        ...
        "description": "Clicking Add to Cart on a sneaker shows no toast, badge update, or aria-live announcement — users assume the click failed.",
        "suggestion": "Add a toast ('Air Max 90 size 10 added to cart'), update the cart badge count, and wrap in an aria-live='polite' region.",
    },
    {
        "selector": "span.stock-indicator",
        "xpath": "/html/body/main/div[3]/span",
        "bounding_box": {"x": 820, "y": 300, "w": 120, "h": 18},
        "severity": "medium",
        "category": "a11y",
        "title": "Low-stock badge uses color alone",
        "description": "The 'Only 2 left!' indicator is red text with no icon or text prefix, inaccessible to colorblind users and not announced as urgent.",
        "suggestion": "Prefix with a warning icon + aria-label='Low stock warning: only 2 pairs left'.",
    },
    {
        ...
        "description": "Price text ($89.99) uses #888 on #FFF (contrast 3.5:1), below AA for normal text.",
        "suggestion": "Darken to #555 or bolden the price to meet 4.5:1.",
    },
    {
        ...
        "description": "Breadcrumb nav (Home > Men's > Running > Air Zoom Pegasus 40) lacks `aria-label='Breadcrumb'` for screen-reader context.",
